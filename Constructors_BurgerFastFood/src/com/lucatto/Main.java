...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        if (!(Burger.hasBeverage(beverageType)) && !(Burger.hasFries(friesSize))) {
            System.out.println("Total: $" + + Burger.calculateBurgerPrice(new Burger(burgerType, toppingsQuantity)) + ".");
            return;
        }
        if (!(Burger.hasBeverage(beverageType)) && (Burger.hasFries(friesSize))) {
            System.out.println("Total: $" + Burger.calculateBurgerPrice(new Burger(burgerType, toppingsQuantity, Burger.intFriesToStringFries(friesSize))) + ".");
            return;
        }
        if ((Burger.hasBeverage(beverageType)) && !(Burger.hasFries(friesSize))) {
            System.out.println("Total: $" + Burger.calculateBurgerPrice(new Burger(burgerType, toppingsQuantity, beverageType)) + ".");
            return;
        }
        System.out.println("Total: $" + Burger.calculateBurgerPrice(new Burger(burgerType, toppingsQuantity, beverageType, Burger.intFriesToStringFries(friesSize))) + ".");
        scanner.close();
    }
